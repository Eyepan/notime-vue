    ...
    <div id="sidebar" class="lg:w-1/6 md:w-1/4 h-full p-2 bg-slate-400 rounded-l-md">
      ...
      <div v-for="page in notebook.pages" class="mt-2">
        <button
          ...
          currentPage = notebook.pages.filter((page) => page.id == currentPageID)[0];
                      ">
          {{ page.title }}
        </button>
        <button @click="deletePage(page.id)"
          class="w-[calc((100%_-_1rem)*0.2)] ml-2 focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
          🗑️
        </button>
      </div>
